 ...
 *   - EventInstance.producer_identity = the identity of whoever produced this event
 *     (e.g. 'agent:hack-start-agent', 'human:christiano', 'system:ci-runner')
 *   - CloudEventEnvelope.source = the stable technical URI of the emitting context
 *     (e.g. 'prodops://payments-api/runtime', 'prodops://payments-api/github-actions')
 *
 * source is NEVER derived from producer_identity.
 * source is a deployment-time concern; producer_identity is an event-schema field.
 *
 * Event Type format: <Namespace>.<Subject>.<Action>[.<Qualifier>]
 *   Correct:   'Delivery.Hack.Started'
 *   Incorrect: 'Delivery.Hack.Phase Lifecycle.Started'  ← Event Category must NOT appear in type
 */
import type {
  OperationalEvent,
  EventId,
...
const context: CloudEventEncodingContext = {
  source: 'prodops://payments-api/runtime' as CloudEventSource,
};
...
const encoder: CloudEventEncoder = {
  encode(e: OperationalEvent, ctx: CloudEventEncodingContext): CloudEventEnvelope<OperationalEvent> {
    return {
      specversion: '1.0',
      id: e.id,
      type: e.event_type,
