...
| Create invoice via Credit Card (Hosted) | Extends payment methods with hosted card, without PCI exposure in the backend. | Checkout, Payments, Asaas provider, confirmation webhook. | OBC at `prodops/artifacts/obcs/credit-card-authorization-confirmation.md`. BDD Feature at `prodops/artifacts/bdd/credit-card-authorization-confirmation.feature`. Approved on 2026-07-07 (EXP-003 + EXP-001 hosted slice). Ready for Bootstrap + Hack. |
